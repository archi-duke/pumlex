...
| 드래그를 했는데 `' @startmeta` 가 안 박힘 | ✓ 클릭이 누락됐을 가능성. 드래그만으로는 소스가 안 바뀐다 — 인라인 에디터의 ✓ 적용 버튼을 눌러야 `WorkspaceEdit` 가 실행되고 메타가 임베드된다. (드래그가 전혀 없는 ✓ 클릭은 의도적으로 메타를 추가하지 않음 — 의미 있는 layout 변경이 있을 때만 임베드.) |
